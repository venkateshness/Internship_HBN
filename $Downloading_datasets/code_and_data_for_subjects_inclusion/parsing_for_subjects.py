...
os.chdir(
    "/homes/v20subra/S4B2/$Downloading_datasets/code_and_data_for_subjects_inclusion"
)
...
df_freesurfer_sliced = [
    df_freesurfer[df_freesurfer.columns[0]].values[i][31:-1]
...
for i in range(len(total_subjects)):
    path_to_file = (
        "/users2/local/Venkatesh/HBN/%s/RestingState_data.csv" % total_subjects[i]
    )
    path_to_file_video = (
        "/users2/local/Venkatesh/HBN/%s/Video3_event.csv" % total_subjects[i]
    )
