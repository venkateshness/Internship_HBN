...
#A very nice overview of the Source Localization mechanism : https://mne.tools/stable/overview/cookbook.html
...
def fire_2(raw,fwd_model,subject,epochs):
    ...
    return inverse.epochs(epochs,inverse_operator)
...
def averaging_by_parcellation(sub):
    ...
    for i in list(set(atlas['labels_R']))[:-1]:
        l.append(np.mean(sub.rh_data[np.where(i== atlas['labels_R'])],axis=0))

    for i in list(set(atlas['labels_L']))[:-1]:
        l.append(np.mean(sub.lh_data[np.where(i== atlas['labels_L'])],axis=0))
...
# %%
np.savez_compressed('/users2/local/Venkatesh/Generated_Data/25_subjects_new/envelope_signal_bandpassed_low_high_beta',**bands)
