...
framewise_probs = np.load("/homes/v20subra/S4B2/AutoAnnotation/framewise_probs.npz")[
    "framewise_probs"
]
...
isc_result = np.load(
    ...
    "sourceISC"
]  # CCA on the eLORETA signal
noise_floor_source = np.load(
...
significance = np.where(
    np.max(np.array(noise_floor_source)[:, :3, :], axis=0) < isc_result[:3]
)[1]
unique_samples = np.unique(significance)
significance_sampled.append(1)
significance_sampled.append(
    unique_samples[np.where(np.diff(unique_samples) > 1)[0] + 1]
)
len(np.hstack(significance_sampled))
#%%
...
def pre_stimulus_average(
    ...
    return np.average(
        array_to_average_on[
            (timeframe * times - _500_ms_in_samples) : (timeframe * times)
        ],
        axis=0,
    )
...
def post_stimulus_average(array_to_average_on, timeframe, times):
    ...
    return np.average(
        array_to_average_on[
            (timeframe * times) : (timeframe * times + _500_ms_in_samples)
        ],
        axis=0,
    )
...
def entire_stimulus_average(array_to_average_on, timeframe, times):
    ...
    return np.average(
        array_to_average_on[
            (timeframe * times - _500_ms_in_samples) : (
                timeframe * times + _500_ms_in_samples
            )
        ],
        axis=0,
...
for i in sample_sorted * _1s_in_frames:
    ...
    scene_detection_entire.append(
        len(
            set(
                np.arange(i - frames_in_samples_pre, i + frames_in_samples_post)
            ).intersection(unique_frame_timestamp)
        )
    )
    scene_detection_pre.append(
        len(
            set(np.arange(i - frames_in_samples_pre, i)).intersection(
                unique_frame_timestamp
            )
        )
    )
    scene_detection_post.append(
        len(
            set(np.arange(i, i + frames_in_samples_post)).intersection(
                unique_frame_timestamp
            )
        )
    )
    ...
    if (
        ...
        offset.append(
            relative_scence_change_from_onset[
                np.abs(relative_scence_change_from_onset).argmin()
            ]
        )
        # print(offset)
    else:
...
df_rms = pd.DataFrame(
    {
        "RMS_pre-stim": np.hstack(rms_pre_stim),
        "RMS_post_stim": np.hstack(rms_post_stim),
        "RMS_entire_event": np.hstack(rms_entire_event),
    }
)
...
def clustering(which_cluster, cluster_labels=None, range_n_clusters=range_n_clusters):
    ...
    if which_cluster == "sub_cluster":
        # VT = VarianceThreshold(threshold=threshold)
        the_df_transformed_single_cluster = variance_thresholding(
            the_df_transformed.iloc[np.where(cluster_labels == 0)]
        )
...
cluster_labels_full, center_full = clustering(
    which_cluster="", range_n_clusters=range_n_clusters
)
centroids_for_full = pd.DataFrame(center_full)
centroids_for_full.columns = the_df_transformed.columns
for i in range(3):
    centroids_for_full.iloc[i, -3:] = np.mean(
        the_df_raw_rms.iloc[np.where(cluster_labels_full == i)[0], -3:], axis=0
    ).values


range_n_clusters = [3]
cluster_labels_sub, center_sub, the_df_transformed_single_cluster = clustering(
    which_cluster="sub_cluster",
...
for i in range(3):
    centroids_for_sub.iloc[i, -3:] = np.mean(
        the_df_raw_rms.iloc[np.where(cluster_labels_sub == i)[0], -3:], axis=0
    ).values
...
np.savez(
    file="/homes/v20subra/S4B2/AutoAnnotation/dict_of_clustered_events_19_events.npz",
    **dic_of_groups,
)
...
# %%
the_df_raw_rms.iloc[np.where(cluster_labels_full == 0)].iloc[
    np.where(cluster_labels_sub == 2)
].iloc[:, -3:]
