...
def pli_graph(band, fmin, fmax):
    stc = np.load(
        "/users2/local/Venkatesh/Generated_Data/25_subjects_new/rs_STC_envelope.npz"
    )[f"{band}"]
    ...
    for subject in range(len(stc_filtered)):
        ...
        pli = mne_connectivity.spectral_connectivity_epochs(
            np.expand_dims(stc_filtered[subject], 0),
            method="pli",
            sfreq=125,
            fmin=fmin,
            fmax=fmax,
...
# %%
np.savez_compressed(
    "/users2/local/Venkatesh/Generated_Data/25_subjects_new/graph_space/pli_graph_21_subjects",
    **pli_graph_dict,
)
# %%
