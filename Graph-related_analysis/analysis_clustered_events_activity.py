...
envelope_signal_bandpassed_bc_corrected = np.load(
    f"/users2/local/Venkatesh/Generated_Data/25_subjects_new/eloreta_cortical_signal_thresholded/bc_and_thresholded_signal/{event_type}/0_percentile_with_zscore.npz"
)
...
for i in range(1, 25):
    if os.path.isfile(
        f"/users2/local/Venkatesh/HBN/CPAC_preprocessed/sub-{total_subjects[i]}_ses-1/functional_to_standard/_scan_rest/_selector_CSF-2mmE-M_aC-CSF+WM-2mm-DPC5_M-SDB_P-2_BP-B0.01-T0.1_C-S-1+2-FD-J0.5/bandpassed_demeaned_filtered_antswarp.nii.gz"
    ):
...
for labels, signal in envelope_signal_bandpassed_bc_corrected.items():
    ...
    envelope_signal_bandpassed_bc_corrected_21_subjects[
        f"{labels}"
    ] = envelope_signal_bandpassed_bc_corrected[f"{labels}"][
        subjects_data_available_for
    ]
...
smoothness_computed_file = np.load(
    f"/users2/local/Venkatesh/Generated_Data/25_subjects_new/graph_space/smoothness_trial_wise_for{event_type}{graph}.npz"
)
...
for labels, signal in smoothness_computed_file.items():
    ...
    smoothness_computed[f"{labels}"] = smoothness_computed[f"{labels}"][
        subjects_data_available_for
    ]
...
def yeo_network_wise_setup(network):  # Network wise
    ...
    for (
        ...
        dic_of_envelope_signals_unthresholded.items()
    ):  # take the cortical unthresholded signal
        ...
        for subject in range(subjects):  # each subject
            ...
            for cluster_group in range(number_of_clusters):  # each cluster
                ...
                per_subject_per_cluster_group_cortical_signal = np.array(signal)[
                    subject, cluster_group, :, :
                ]  # take the signal of interest
                assert np.shape(per_subject_per_cluster_group_cortical_signal) == (
                    regions,
                    seconds_per_event,
                )

                indices_of_roi_belonging_network_group = np.where(
                    np.array(match) == network
                )[
                    0
                ]  # indices of ROI given a network
                rois_belonging_to_a_network_averaged = np.mean(
                    per_subject_per_cluster_group_cortical_signal[
                        indices_of_roi_belonging_network_group
                    ],
                    axis=0,
                )
                assert np.shape(rois_belonging_to_a_network_averaged) == (
                    seconds_per_event,
                )
        ...
        assert np.shape(cortical_signal_baseline_corrected) == (
            subjects,
            number_of_clusters,
            seconds_per_event,
        )

        dic_of_cortical_signal_baseline_corrected[
            f"{labels}"
        ] = cortical_signal_baseline_corrected
...
for i in range(1, 8):
    dic_of_cortical_signal_baseline_corrected_nw[f"{i}"] = yeo_network_wise_setup(
        network=i
    )
...
for band in bands:  # cortical signal bands
    dic_of_cortical_signal_baseline_corrected_nw["8"][f"{band}"] = np.squeeze(
        np.mean(
            dic_of_envelope_signals_unthresholded[f"{band}"][
                :, :, [aud_ROI_label, aud_ROI_label + 179], :
            ],
            axis=2,
        )
    )
...
def plotting(band):
    ...
    for i in [i for i in range(9) if i != 5]:  # neglecting Limbic network
        ...
        for cluster_group in range(number_of_clusters):
            ...
            if i > 0:  # rest for cortical activity
                ...
                label_band = list(dic_of_cortical_signal_baseline_corrected_nw.keys())[
                    i - 1
                ]
                signal = np.array(
                    dic_of_cortical_signal_baseline_corrected_nw[f"{label_band}"][
                        f"{band}"
                    ]
                )[:, cluster_group, :]
            ...
            plt.xticks(
                np.arange(0, video_duration, pre_stim_in_samples),
                labels=["-200", "0", "200", "400"],
            )
            ...
            plt.axvline(
                pre_stim_in_samples, label="Onset (ISC)", c="g", linestyle="dashed"
            )

            if counter in [
                idx for idx in range(1, number_of_clusters * 8, number_of_clusters)
            ]:
            ...
            for samples in range(pre_stim_in_samples, seconds_per_event):
                ...
                pvalues[samples - pre_stim_in_samples] = scipy.stats.ttest_1samp(
                    signal_post_onset, popmean=0
                )[1]

            # pvalues_corrected = multipletests(pvalues, method = "bonferroni")[1]
            t, c, c_pv, h0 = mne.stats.permutation_cluster_1samp_test(
                signal, adjacency=None, n_permutations=1000
            )
    ...
    fig.suptitle(
        f"{graph} Graph / 21 Subjects / {event_type}/ GSV + Cortical activity in Yeo NW/ {band} band / corrected, n_perm = 1000"
    )
    fig.supxlabel("latency (ms)")
    fig.savefig(
        f"/homes/v20subra/S4B2/Graph-related_analysis/ERD_baseline_C_zscore_{graph}/intra-cluster/{event_type}/{band}.jpg"
    )
...
def band_wise_stats(band):
    ...
    for nw in [nw for nw in range(9) if nw != 5]:
        ...
        if nw > 0:
            signal_for_anova = np.array(
                dic_of_cortical_signal_baseline_corrected_nw[f"{nw}"][f"{band}"]
            )
            signal_for_anova_ = [
                signal_for_anova[:, 0, :],
                signal_for_anova[:, 1, :],
        ...
        if nw > 0:
            F_obs, c, c_pv, H0 = mne.stats.permutation_cluster_test(
                signal_for_anova_,
                tail=0,
                adjacency=None,
                out_type="mask",
        ...
        if nw == 0:
            F, pvals = scipy.stats.f_oneway(
                signal_for_anova_[0], signal_for_anova_[1], signal_for_anova_[2]
            )
            ...
            if len(c) > 0:
                ...
                if len(idx) > 0:
                    for i in idx:
                        print(i[0])
                        plt.axvspan(
                            c[i[0]][0].start, c[i[0]][0].stop, color="green", alpha=0.1
                        )
        plt.xticks(
            np.arange(0, video_duration, pre_stim_in_samples),
            labels=["-200", "0", "200", "400"],
    ...
    fig.suptitle(
        f"{graph} Graph / 21_subjs / ANOVA; corrected with n_perms = 1000 / 95%CI / band = {band} / # event = {event_type}"
    )
    fig.savefig(
        f"/homes/v20subra/S4B2/Graph-related_analysis/ERD_baseline_C_zscore_{graph}/Anova_group_wise/{band}"
    )
...
def plotting_pairs(band):
    ...
    for i in [i for i in range(9) if i != 5]:  # neglecting Limbic network
        ...
        for cluster_group in range(len(cluster_pairs)):
            ...
            plt.style.use("fivethirtyeight")
            if i == 0:  # Plot GSV on top
                signal_cluster_1 = smoothness_computed[f"{band}"][
                    :, :, cluster_pairs[cluster_group][0] - 1
                ]
                signal_cluster_2 = smoothness_computed[f"{band}"][
                    :, :, cluster_pairs[cluster_group][1] - 1
                ]
            ...
            if i > 0:  # rest for cortical activity
                ...
                signal_cluster_1 = np.array(
                    dic_of_cortical_signal_baseline_corrected_nw[f"{label_band}"][
                        f"{band}"
                    ]
                )[:, cluster_pairs[cluster_group][0] - 1, :]
                signal_cluster_2 = np.array(
                    dic_of_cortical_signal_baseline_corrected_nw[f"{label_band}"][
                        f"{band}"
                    ]
                )[:, cluster_pairs[cluster_group][1] - 1, :]
            ...
            sem_and_plotting(
                signal_cluster_1,
                c="orange",
                label=f"C {cluster_pairs[cluster_group][0]}",
            )
            sem_and_plotting(
                signal_cluster_2, label=f"C {cluster_pairs[cluster_group][1]}"
            )
    ...
    fig.supylabel("relative variation")
    fig.suptitle(
        f"{graph} Graph / 21 subjs / {event_type}/ Paired t-test {band} band / fdr-corrected and uncorrected (in cyan)"
    )
    fig.supxlabel("latency (ms)")
    fig.savefig(
        f"/homes/v20subra/S4B2/Graph-related_analysis/ERD_baseline_C_zscore_{graph}/paired/{event_type}/{band}.jpg"
    )
