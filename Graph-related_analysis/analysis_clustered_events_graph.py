...
def dicting(percentile):
    ...
    envelope_signal_thresholded = np.load(
        f"/users2/local/Venkatesh/Generated_Data/25_subjects_new/eloreta_cortical_signal_thresholded/bc_and_thresholded_signal/{percentile}_percentile.npz"
    )
...
for labels_outer, signal_outer in dic_of_all_signals.items():
    for labels_inner, signal_inner in signal_outer.items():
        ...
        for event_group in range(number_of_clusters):
            ...
            event_level.append(
                smoothness_computation(signal_inner[:, event_group, :, :])
            )
        dic_of_smoothness_signals[f"{labels_outer}"][f"{labels_inner}"] = event_level
        assert np.shape(
            dic_of_smoothness_signals[f"{labels_outer}"][f"{labels_inner}"]
        ) == (number_of_clusters, video_duration, subjects)
...
def plotting(band):
    ...
    for labels_outer, signal_outer in dic_of_smoothness_signals.items():
        ...
        assert np.shape(signal_band_wise_percentile_wise) == (
            number_of_clusters,
            subjects,
            seconds_per_event,
        )
        ...
        for cluster_group in range(number_of_clusters):
            ...
            if c in [idx for idx in range(1, 41, 5)]:
                ...
                plt.ylabel(
                    f"{percentiles[  counter ]}_percentile",
                    rotation=25,
                    size="large",
                    color="r",
                )
    ...
    fig.suptitle(
        f"GSV at varying threshold of the cortical signal (FDR-Corrected pvalues) SC -- {band} band"
    )
