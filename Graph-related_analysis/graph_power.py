...
def master(band):
    ...
    """
    one_freq = list()
    GFTed_cortical_signal = [
        G.gft(np.array(band[i])) for i in range(subjects)
    ]  # Applying GFT on the cortical signal, for all the subjects
    ...
    for i in range(len(index)):  # looping over each trials
        indices_whole = np.hstack(
            [np.arange(index[i] * fs - fs, index[i] * fs + 2 * fs)]
        )  # taking data from -1 to +2 seconds, thus 3s in total
        print(
            "the whole-indices length", np.shape(indices_whole)
        )  # the indices for the said 3 seconds

        low_freq_whole = slicing_time(GFTed_cortical_signal_low_freq, indices_whole)
        med_freq_whole = slicing_time(GFTed_cortical_signal_medium_freq, indices_whole)
        high_freq_whole = slicing_time(GFTed_cortical_signal_high_freq, indices_whole)
        ...
        assert np.shape(low_freq_whole) == (
            subjects,
            len(low_freq_range),
            len(indices_whole),
        ...
        assert np.shape(low_freq_f_summed_whole) == (
            subjects,
            len(indices_whole),
        )  # Check for the integrity of summing the frequency
...
def averaging_ERD(which_freq, env_band):
    ...
    dic2["gFreqs"] = np.squeeze(
        np.concatenate([[f"{env_band}"] * 2 * subjects])
    ).T  # len(pre_stim) + len(post_stim) =  2 * len(subjects)
...
for i in range(
    3
):  # iterating over graph frequencies; 0 = low, 1 = Med; 2 = High gFreqs
    the_returned, averaged, sem = averaging_ERD(which_freq=i, env_band=env_bands[i])

    df_for_plotting_purpose = pd.concat(
        [pd.DataFrame(the_returned), df_for_plotting_purpose], ignore_index=True
    )
    ax = fig.add_subplot(a, b, i + 1)
    ax.plot(averaged, color="r")
    ax.fill_between(
        range(total_duration_in_samples), averaged - sem, averaged + sem, alpha=0.2
    )
    ax.axvline(125)
    ...
    if i == 0:
        ...
        np.savez_compressed(
            f"/users2/local/Venkatesh/Generated_Data/25_subjects_copy_FOR_TESTING/power_smoothness_4_in_one_plot/gPower",
            **dic_to_write,
        )
...
fig.suptitle(f"ERD of graph power for the averaged trials across subjects -- {band}")
...
]  #  converting the indices into string for parsing from the dictionary
dic_parse_freq_wise = [
    dic.get(key)[0] for key in index_in_str
]  # 0 = Low gfreq; 1 and 2 = Med and High gFreq respectively
...
for i in range(7):
    ...
    plt.fill_between(
        ...
        np.array(conf_int_a).T[:, 0],
        np.array(conf_int_a).T[:, 1],
        alpha=0.2,
        label="95% CI",
    )
    plt.xticks(
    ...
    plt.axvspan(
        xmin=0, xmax=baseline_duration_of_900ms_in_samples, color="r", alpha=0.2
    )
    c += 1
    plt.legend()
