...
def fullstack(graph):
    ...
    for labels, signal in dic_of_envelope_signals_thresholded.items():
        ...
        for event in range(number_of_clusters):
            ...
            signal_normalized = (
                signal_for_gsv / np.diag(laplacian_kalofias)[np.newaxis, :, np.newaxis]
            )
            placeholder_gsv.append(
                smoothness_computation(signal_normalized, laplacian_kalofias)
            )
...
def plot(ax, data):
    ...
    if ax == 0:
        ...
        plt.fill_between(
            range(seconds_per_event), mean - sem, mean + sem, alpha=0.2, color="b"
        )

    elif ax == 1:
        plt.plot(mean, color="r", label="Original SC")
        plt.fill_between(
            range(seconds_per_event), mean - sem, mean + sem, alpha=0.2, color="r"
        )
