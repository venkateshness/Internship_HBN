...
def parcellation(mask, img):
    ...
    glassermasker = NiftiLabelsMasker(
        labels_img=path_Glasser, mask_img=mask, standardize=True
    )
...
def connectivitymeasure(which_subject_after_parcellation):
    ...
    correlation_matrix = correlation_measure.fit_transform(
        [which_subject_after_parcellation]
    )[
        0
    ]  # 25 indiv
...
envelope_signal_bandpassed_bc_corrected = np.load(
    f"/users2/local/Venkatesh/Generated_Data/25_subjects_new/eloreta_cortical_signal_thresholded/bc_and_thresholded_signal/{event_type}/0_percentile.npz"
)

idx_for_the_existing_subjects = np.argwhere(
    np.isin(total_subjects, subjects_data_available_for)
).ravel()
...
for labels, signals in envelope_signal_bandpassed_bc_corrected.items():
    envelope_signal_bandpassed_bc_corrected_sliced[
        f"{labels}"
    ] = envelope_signal_bandpassed_bc_corrected[f"{labels}"][
        idx_for_the_existing_subjects
    ]
...
parcellated_data = np.load(
    "/users2/local/Venkatesh/Generated_Data/25_subjects_new/graph_space/21_subject_fmri_parcellated.npz"
)["parcellated_data"]
...
for labels, signal in envelope_signal_bandpassed_bc_corrected_sliced.items():
    ...
    for subject in range(subjects):
        ...
        signal_normalized = (
            envelope_signal_bandpassed_bc_corrected_sliced[f"{labels}"][subject]
            / np.diag(laplacian)[np.newaxis, :, np.newaxis]
        )
...
np.savez_compressed(
    "/users2/local/Venkatesh/Generated_Data/25_subjects_new/graph_space/21_fmri_graph",
    laplacian=laplacian_for_21_subjects,
)
#%%
...
for labels, signal in envelope_signal_bandpassed_bc_corrected_sliced.items():
    ...
    for i in range(subjects):
        learnt_weights = np.load(
            "/users2/local/Venkatesh/Generated_Data/25_subjects_new/graph_space/pli_graph_21_subjects.npz"
        )[f"{labels}"][i]
        ...
        signal_normalized = (
            envelope_signal_bandpassed_bc_corrected_sliced[f"{labels}"][i]
            / np.diag(degree)[np.newaxis, :, np.newaxis]
        )

        gsv_resting_state.append(
            smoothness_computation(signal_normalized, laplacian_pli)
        )
...
samples, sample_rate = librosa.load(
    "/homes/v20subra/S4B2/Despicable Me-HQ.wav", sr=None
)
...
for i, j in events.items():
    ...
    for second in j:
        group.append(
            np.sqrt(
                np.square(
                    np.average(samples_normed[fs * second : fs * second + int(fs / 2)])
                    - np.average(
                        samples_normed[fs * second - int(fs / 5) : fs * second]
                    )
                )
            )
        )
...
for labels in gsv_original_all_bands.keys():
    ...
    for event_no in range(number_of_clusters):
        ...
        mean_orig, sem_orig = mean_std(
            np.array(gsv_original_all_bands[f"{labels}"])[:, :, event_no]
        )
        ...
        plt.plot(mean_orig)
        plt.fill_between(
            range(video_duration), mean_orig - sem_orig, mean_orig + sem_orig, alpha=0.2
        )
        ...
        _, pvalues = scipy.stats.ttest_rel(
            np.array(gsv_resting_state_all_bands[f"{labels}"])[
                :, pre_stim_in_samples:, event_no
            ],
            np.array(gsv_original_all_bands[f"{labels}"])[
                :, pre_stim_in_samples:, event_no
            ],
        )
...
fig.suptitle(
    f"{event_type} / GSV on fMRI RSFC / cortical signal = unthresholded, with Bonferroni correction"
)
