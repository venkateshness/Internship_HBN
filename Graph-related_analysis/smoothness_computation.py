...
for labels, signals in dict_of_unthresholded_signals_for_all_bands.items():
    dict_of_unthresholded_signals_for_all_bands_sliced[
        f"{labels}"
    ] = dict_of_unthresholded_signals_for_all_bands[f"{labels}"][
    ...
    for subject in range(subjects):  # Subject-wise
        ...
        for event_label, event_time in clusters.items():  # Cluster-wise
            ...
            for event in event_time:  # event-wise
                ...
                signal_sliced_bc_normalized = (
                    signal_sliced_bc / np.diag(laplacian)[:, np.newaxis]
                )
                smoothness = smoothness_computation(
                    signal_sliced_bc_normalized, laplacian=laplacian
                )
...
# %%
np.savez_compressed(
    f"/users2/local/Venkatesh/Generated_Data/25_subjects_new/graph_space/smoothness_trial_wise_for{event_type}{graph_type}_PLI",
    **dict_of_sliced_bc_averaged,
)
