...
def plot(band, threshold_pc):
    ...
    for i in range(len(trials)): # looping over each trials
            ...
            plotting.plot_surf(U0_brain,title=f'Trial {i+1}',colorbar=True,plot_abs=False,display_mode='lzr', axes=ax.flatten()[i], cmap='hsv', figure=fig)

            print('shape', np.shape(subjects_averaged[peak].T))

    fig.suptitle(f'Env. {band} on the brain, thresholded at {threshold_pc} %ile', fontsize = 20)
    fig.savefig(f'/homes/v20subra/S4B2/3Source_Inversion_full_stack/Results/topoplots/{band}.jpg')
    plt.show()
...
plotting.plot_surf(surf_mesh=fsaverage['infl_left'],surf_map=np.ones(20484,),hemi = ['left','right'])
