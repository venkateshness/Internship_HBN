...
def CCAfy(
    epochs,
    how_many_channels_for_CCA,
    need_noise_floor,
    how_many_subjects,
    how_many_trials_for_noise_floor,
):
    dic = dict()
    ...
    for cond_key, cond_values in dic.items():
        isc_results[str(cond_key)] = dict(
            zip(
                ...
                CCA.apply_cca(cond_values, W, 125),
            )
        )
    ...
    if need_noise_floor:
        ...
        def process(i):
            ...
            for subjects in range(how_many_subjects):
                ...
                rng.shuffle(
                    np.swapaxes(
                        dic["condition1"][subjects, :, :].reshape(
                            how_many_channels_for_CCA, 34, 625
                        ),
                        0,
                        1,
                    )
            ...
            return dict(
                zip(
                    ["ISC", "ISC_persecond", "ISC_bysubject", "A"],
                    CCA.apply_cca(dic["condition1"], W, 125),
                )
            )["ISC_persecond"]

        isc_noise_floored = Parallel(n_jobs=NB_CPU - 1, max_nbytes=None)(
            delayed(process)(i) for i in tqdm(range(how_many_trials_for_noise_floor))
        )
