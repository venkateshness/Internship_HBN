...
brain_plot(np.reshape(to_plot,(1,360)),'ff',expand=False)
...
activation_time_series(high,'Time-Series Activations for the High ISC Period',1,360,10,1,360,'AVG')# (differenced high with low & averaged through subjects )
...
signal=np.reshape(to_plot,(1,360)) # add dimension 1 to signal array
U0_brain = signals_to_img_labels(signal,path_Glasser,mnitemp['mask'])
plotting.plot_glass_brain(U0_brain,title='Statistical significance on eLORETA activations in ROIs',colorbar=True,plot_abs=False,cmap='seismic',display_mode='lzr',symmetric_cbar=False,figure=fig,axes=g2)
...
signal=data # add dimension 1 to signal array
U0_brain = signals_to_img_labels(signal,path_Glasser,mnitemp['mask'])
plotting.plot_glass_brain(U0_brain,title='Statistical significance (t-values) on eLORETA activations between Low and High ISC',colorbar=True,plot_abs=False,cmap='seismic',display_mode='lzr',symmetric_cbar=True,figure=fig,axes=g3)
