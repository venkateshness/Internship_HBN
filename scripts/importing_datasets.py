...
resting_state_sub2_raw,resting_state_sub2_events = preparation_resting_state('NDARDX770PJK','Rest')
#resting_state_sub4_raw,resting_state_sub4_events = preparation_resting_state('NDARGY054ENV','Rest')
resting_state_sub2_raw.save('Generated_Data/importing/resting_state_sub2_raw.fif',overwrite=True)
np.savez('Generated_Data/importing/resting_state_sub2_events.npz',sub2 = resting_state_sub2_events)


'''Video-watching'''
subject_list = ['NDARCD401HGZ','NDARDX770PJK', 'NDAREZ098ZPE', 'NDARGY054ENV', 'NDARMR242UKQ', 'NDARRD720XZK', 'NDARTR840XP1', 'NDARXJ696AMX', 'NDARYY218AGA', 'NDARZP564MHU']
for i in range(1,11):
    sub_raw,sub_events = preparation(subject_list[i-1],'others')
    sub_raw.save(f'Generated_Data/importing/video_sub{i}_raw.fif',overwrite=True)
    np.savez(f'Generated_Data/importing/video_sub{i}_events.npz',sub2 = sub_events)