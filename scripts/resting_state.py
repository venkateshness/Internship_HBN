...
for i in range(0, 5):  # 0 is 90 = beginning of EEG, so skipped
    ...
    print("\t\t\tEVENT IN THE GRAPH BELOW IS = {}".format(
        resting_state_sub2_events[i][2]))
    ...
    mne.viz.plot_raw_psd(resting_state_sub2_raw, tmin=resting_state_sub2_events[i][0]/500,
                         tmax=resting_state_sub2_events[i+1][0]/500, fmax=40, picks=['E70', 'E75', 'E83'])
...
'''Noise Covariance'''
epochs_for_noice_covariance = epochs_slicing.epochs(resting_state_sub2_raw, resting_state_sub2_events, [
                                                    20, 30, 90], tmin=0, tmax=0.95, fs=500, epochs_to_slice='90')
noise_covariance = mne.compute_covariance(
    epochs_for_noice_covariance[0],  method='auto')
